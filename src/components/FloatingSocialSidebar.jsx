...
export default function FloatingSocialSidebar() {
  return (
    /* Changes made for a larger size:
      - Changed width from w-[46px] to w-[56px]
      - Increased horizontal padding from px-3 to px-4
      - Tweaked top placement to top-[28%] to account for the extra length
    */
    <div className="fixed left-0 top-[28%] z-50 flex flex-col items-center bg-white rounded-r-[24px] py-8 px-4 border border-l-0 border-gray-200/80 shadow-[6px_0_20px_rgba(0,0,0,0.06)] w-[56px]">
      ...
      {/* Increased text size from text-[10px] to text-[12px] 
        and added tracking-widest for clean spacing 
      */}
      ...
      {/* Slightly widened the orange accent strip from 3px to 4px */}
      <div className="absolute left-0 top-0 bottom-0 w-[4px] bg-[#FFAE34] rounded-r-sm" />

      {/* Increased spacing between social icons from space-y-4 to space-y-5 */}
      <div className="flex flex-col space-y-5 text-gray-400 items-center justify-center w-full">
        <a href="#" className="hover:text-[#FFAE34] transition-colors duration-200 transform hover:scale-110">
          {/* Increased icon width/height from 15px to 18px */}
        ...
        <a href="#" className="hover:text-[#FFAE34] transition-colors duration-200 transform hover:scale-110">
          {/* Increased icon width/height from 14px to 17px */}
